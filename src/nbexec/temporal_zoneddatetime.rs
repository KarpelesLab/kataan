...
impl<'a> Interp<'a> {
    ...
    pub(crate) fn zoneddatetime_construct(
        ...
        Err(self.temporal_todo("ZonedDateTime constructor"))
    }
    ...
    pub(crate) fn zoneddatetime_method(
        ...
        Err(self.temporal_todo(&alloc::format!("ZonedDateTime.prototype.{method}")))
    }
    ...
    pub(crate) fn zoneddatetime_getter(
        ...
        Err(self.temporal_todo(&alloc::format!("ZonedDateTime getter {name}")))
    }

    /// A `Temporal.ZonedDateTime.<static>()` call. `Ok(None)` = not a recognised static.
