...
pub enum SnapCell {
    ...
    TypedArray {
        ...
        buffer: SnapVal,
        /// reference to the `[[ViewedArrayBuffer]]` object cell
        array_buffer: SnapVal,
...
pub fn capture(realm: &Realm, roots: &[Handle]) -> Snapshot {
    ...
    while pos < order.len() {
        ...
        let cell = if let Some(s) = realm.string_value(h) {
            ...
            // The `[[ViewedArrayBuffer]]` object is interned as a child so its
            // identity (shared across sibling views) round-trips.
            let array_buffer = realm.typed_array_object(h).map_or(SnapVal::Null, |b| {
                snap_val(
                    NanBox::handle(b.to_raw()),
                    &mut index_of,
                    &mut order,
    ...
    for (idx, (cell, h)) in snap.cells.iter().zip(&handles).enumerate() {
        match cell {
            ...
            SnapCell::TypedArray {
                ...
                // Rebind the `[[ViewedArrayBuffer]]` object so `.buffer` identity
                // (shared across sibling views) is preserved.
                if let Some(b) = resolve(array_buffer, &handles).as_handle() {
                    realm.set_typed_array_object(*h, Handle::from_raw(b));
                }
            }
            SnapCell::Array(vals) => {
...
/// The file-backed snapshot reload path for heap snapshots: a `KSNP` artifact is
/// read into an owned buffer and deserialized, then restored.
#[cfg(all(feature = "std", target_os = "linux", target_arch = "x86_64"))]
mod mmap {
    use super::{deserialize, restore};
    use crate::heap::Handle;
    use crate::realm::Realm;
    use alloc::vec::Vec;

    /// Reloads the snapshot at `path` from disk and restores it into `realm`,
    /// returning the new root handles.
    ///
    /// Snapshots are treated as untrusted input, so the file is read into an
    /// owned `Vec<u8>` rather than memory-mapped: the deserializer copies the
    /// bytes into owned structs regardless, so zero-copy buys nothing here, and
    /// reading up front avoids the SIGBUS hazard a concurrent truncation of an
    /// active mapping would cause (a fault `catch_unwind` cannot intercept).
    ///
    /// # Errors
    /// `io::Error` on a failed open/read, or `InvalidData` wrapping the
    /// [`SnapError`](super::SnapError) if the bytes don't deserialize.
    pub fn restore_file(realm: &mut Realm, path: &str) -> std::io::Result<Vec<Handle>> {
        ...
        let snap = deserialize(&bytes).map_err(|e| {
...
mod tests {
    ...
    fn snapshots_typed_array_views_preserving_aliasing() {
        ...
        let abuf = realm.new_object(); // the shared ArrayBuffer object
        ...
        // Both restored views share the *same* restored ArrayBuffer object too.
        assert_eq!(realm2.typed_array_object(u2), realm2.typed_array_object(f2));
        assert!(realm2.typed_array_object(u2).is_some());
